...
streaming_version = """    suspend fun getSeriesDetail(seriesId: String): Pair<SeriesDetailInfo?, Map<String, List<Episode>>> = withContext(Dispatchers.IO) {
        ...
        return@withContext try {
            ...
            response.body!!.charStream().use { reader ->
                ...
                val rootMap: Map<String, JsonElement> = gson.fromJson(reader, type)
                
                val infoElement = rootMap["info"]
                val info = if (infoElement != null && infoElement.isJsonObject) {
                    gson.fromJson(infoElement, SeriesDetailInfo::class.java)
                } else null

                val episodesMap = mutableMapOf<String, List<Episode>>()
                val epElement = rootMap["episodes"]
                if (epElement != null && epElement.isJsonObject) {
                    ...
                    for (seasonKey in epObj.keySet()) {
                        try {
                            ...
                            val list: List<Episode> = gson.fromJson(seasonArr, epType) ?: emptyList()
