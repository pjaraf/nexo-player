...
tv_screen = re.search(r'(private fun SeriesDetailTvScreen\([\s\S]*?^})', series_code, re.MULTILINE).group(1)

# Now we transform tv_screen into MovieDetailTvScreen
...
# Now remove bottom row and season picker dialog
# Instead of complex regex, let's just find the exact index
code = re.sub(r'// Bottom Row: Seasons & Episodes \[\+\] \(Only when not in full screen\)[\s\S]*?// \-\-\- DIALOGS \-\-\-', '''// Bottom Row: Movies Carousel (Only when not in full screen)
...
# Remove Season Picker dialog completely
code = re.sub(r'if \(showSeasonPicker\) \{[\s\S]*?\}\n        \}\n    \}', '', code)
