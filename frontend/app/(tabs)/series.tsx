...
export default function SeriesScreen() {
  ...
  const loadCats = useCallback(async (kidsMode: boolean) => {
    try {
      const { data } = await api.get("/series/categories");
  ...
  const loadItems = useCallback(async (cat: string, kidsMode: boolean, kidsCatIds: Set<string>) => {
    setLoading(true);
    try {
      const { data } = await api.get("/series/list", {
        params: cat === "ALL" ? {} : { category_id: cat },
      });
      let list: Series[] = data.series || [];
      if (kidsMode && cat === "ALL") {
  ...
  useEffect(() => {
    ...
    loadItems(active, !!profile?.isKids, ids);
  }, [active, profile, cats, loadItems, ready]);
        ...
        {[{ category_id: "ALL", category_name: "TODAS" }, ...cats].map((c) => {
          ...
          return (
            <Pressable
              ...
              testID={`series-chip-${c.category_id}`}
              style={({ focused }) => [
                styles.chip,
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable
              ...
              testID={`series-item-${item.series_id}`}
              style={({ focused }) => [
                styles.card,
          ...
          ListEmptyComponent={<Text style={styles.empty}>No hay series</Text>}
        />
      )}
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  card: { flex: 1, gap: 6, maxWidth: "33%", borderRadius: radius.md, borderWidth: 2, borderColor: "transparent" },
  cardFocused: {
    borderColor: colors.primary,
    shadowColor: colors.primary,
    shadowOpacity: 0.9,
